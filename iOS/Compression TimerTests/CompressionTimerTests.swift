import XCTest
@testable import CompressionTimer

@MainActor
final class CompressionTimerTests: XCTestCase {
    ...
    func testUpdateReplacesItem() {
        ...
        first.label = "Updated Name"
        store.update(first)
        XCTAssertEqual(store.items.first?.label, "Updated Name")
    }
